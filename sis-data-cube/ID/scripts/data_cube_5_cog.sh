...
COUNTRY=ID
INPUT_DIR="/home/carva014/Work/Code/FAO/GloSIS-dev/sis-data-cube/$COUNTRY/tmp"
OUTPUT_DIR="/home/carva014/Work/Code/FAO/GloSIS-dev/sis-data-cube/$COUNTRY/output"
cd $INPUT_DIR
...
for FILE in *.tif; do
    ...
    if [[ "$BASENAME" == *"GSNM"* || "$BASENAME" == *"OTHER"* ]]; then
        XRES=0.002245788210311 # 250 meters in degrees
        YRES=0.002245788210311 # 250 meters in degrees
    else
...
# gdalbuildvrt -q -separate -input_file_list filelist.txt $COUNTRY-GSOC.vrt
# rm filelist.txt
