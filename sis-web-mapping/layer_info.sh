...
psql -h localhost -p 5432 -U sis -d sis -c "\copy (
        ...
        LEFT JOIN spatial_metadata.url u ON u.mapset_id = m.mapset_id AND u.url_name = 'Download '||l.dimension_des
        WHERE m.country_id = '$COUNTRY_ID'
        ORDER BY p.project_name, l.layer_id
        ) 
TO '/home/carva014/Work/Code/FAO/GloSIS/sis-web-mapping/layer_info_${COUNTRY_ID}.csv' WITH CSV HEADER"
